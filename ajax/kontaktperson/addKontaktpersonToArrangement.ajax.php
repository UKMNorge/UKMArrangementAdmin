...
if($foundMobil != 'null' && $foundMobil != -1) {
    $okp = OmradeKontaktpersoner::getByMobil($foundMobil);
    connectOkpToOmrade($okp, $omrade, $handleCall);
}
...
function connectOkpToOmrade(OmradeKontaktperson $okp, Omrade $omrade, HandleAPICallWithAuthorization $handleCall) {
    try {
    ...
    $handleCall->sendToClient(['message' => 'Kontaktperson lagt til']);
}
